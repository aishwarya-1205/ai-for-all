...
const presets = [
  {
    id: "face",
    label: "Face Restore",
    desc: "Bring blurry faces back to life",
  },
  {
    id: "denoise",
    label: "Denoise",
    desc: "Remove grain & compression artifacts",
  },
  { id: "deblur", label: "Deblur", desc: "Sharpen out-of-focus images" },
  {
    id: "colorize",
    label: "Colorize",
    desc: "Add color to black & white photos",
  },
];
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[280px_1fr] gap-5">
      ...
      >
        <div>
          <p
            className={cn(
              "text-[10px] font-medium uppercase tracking-widest mb-2",
        ...
        <div>
          ...
          <input
            ...
            className={cn(
              ...
              incognitoMode && "accent-slate-400",
            )}
          />
        </div>
        ...
        {/* Upload — muted-foreground/10, hover muted-foreground/30 */}
        <button
        ...
        {/* Restore CTA — keep gradient-accent */}
        <button
          onClick={() => {
            if (!imageUrl) return;
            setProcessing(true);
        ...
        >
          {processing ? (
            ...
            <>
              <Wand2 className="w-3.5 h-3.5" /> Restore
            </>
          )}
        </button>
      </div>
      ...
      >
        {!imageUrl ? (
          <div className="text-center">
            ...
            <p
              className={cn(
                "text-[12px] mt-1.5 max-w-[320px]",
                incognitoMode
                  ? "text-slate-400/40"
                  : "text-muted-foreground/60",
              )}
            >
              Upload a damaged, blurry, or faded image and let Rivinity bring it
              back to life.
            </p>
          </div>
        ) : (
          <>
            ...
            <img
              src={imageUrl}
              alt="restore"
              className="relative max-h-[420px] max-w-full rounded-xl shadow-float object-contain"
            />
            {!processing && (
              <button
                className={cn(
